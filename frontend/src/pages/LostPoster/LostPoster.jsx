...
const LostPoster = () => {
  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        ...
        // Generate poster automatically when component loads
        setTimeout(() => {
          generatePosterImage();
        }, 1000); // Small delay to ensure the DOM is fully rendered
      } catch (err) {
  ...
  const handleShareSocial = async (platform) => {
    ...
    try {
      // Upload poster image to server and get shareable URL
      ...
      switch (platform) {
        ...
        case 'facebook':
          ...
          break;
        case 'twitter':
          window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(response.shareUrl)}&text=${encodeURIComponent('¡Ayuda a encontrar esta mascota perdida!')}`);
          break;
        default:
      ...
      <div className="lost-poster-container">
        <div ref={posterRef} className="lost-poster">
          ...
          <div className="poster-footer">
            <p>¡Tu ayuda es muy importante para reunir a {pet.name} con su familia!</p>
          </div>
        </div>
      </div>
    </div>
